...
- `librime-sbxlm`：供独立的 SbxlmKeyboard target 链接，上游原包就是一个完整 librime 构建（含声笔插件）。本工程不含声笔方案，直接复用刚编好的 librime 副本提供 `rime_*` 符号；副本内部二进制改名为 `librime-sbxlm.a`、去除头文件目录并同步移除 Info.plist 中的 `BinaryPath`/`HeadersPath` 条目，避免与 `librime` 在 Xcode ProcessXCFramework 阶段产生重复产物冲突或指向已删目录的悬空引用；
